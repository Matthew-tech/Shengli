...
def GHMC_Loss(Bins=10,momentum=0,batch_size=100):
    global edges,mmt,bins,shape,acc_sum ## 
    shape=batch_size
    bins=Bins
    ...
    def GHMC_Loss_fixed(y_true,y_pred):
        ...
        weights=tf.py_func(GHMC_loss_pyfunc,[y_true,y_pred,momentum],tf.float32)
...
def GHMC_loss_pyfunc(y_true,y_pred,momentum): 
...
#     new_model.compile(optimizer=adam, loss='categorical_crossentropy', metrics=['accuracy'])
    ghm = GHMC_Loss(Bins=10,momentum=0.1,batch_size=batch_size)
