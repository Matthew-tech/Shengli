...
from data_prepare.data_util import Build_Interpolation_function
...
def generate_cnn_inputs(filepath = '',filename='',total=False):
    ...
    assert os.path.exists(this_file),this_file+'no'
    training_dataset_pos = {}
    training_dataset_neg = {}
    training_dataset_dir = 'data/cnn_train'
    ...
    else: print(this_savedfile_pos+'exits！')
def get_horizon_slice(filepath='',filename='',sgy_info=[]):
    """
    获得filename 所命名的横向切面的测试数据
    :param filepath:  横向切面文件的存放路径
    :param filename:  横向切面文件名
    :param sgy_info:  [sgy_dir, sgy filename]
    :return:
    """
    this_file = os.path.join(filepath,filename)     # 横向切面的文件
    sgyfile = os.path.join(sgy_info[0],sgy_info[1])                     # 地震体文件路径
    #print(os.path.exists(this_file))
    assert os.path.exists(this_file),this_file+'no'
    assert os.path.exists(sgyfile),sgyfile+'no'
    plane_file_pkl = "{}.pkl".format(filename)
    plane_file_pkl_save = os.path.join(filepath,plane_file_pkl)
    # 将切面文件的信息保存下来
    if not os.path.exists(plane_file_pkl_save):
        print('generating: %s'%plane_file_pkl_save)
    ...
    with open(plane_file_pkl_save,'rb') as file:
        ...
        print(len(loc_time_dict))
        #if len(loc_time_dict) != 1068468: return
    horizon_data = []
    horizon_data_dir = 'data/cnn_test'
    if not os.path.exists(horizon_data_dir): os.makedirs(horizon_data_dir)
    horizon_data_filename = "{}_{}.ht".format(sgy_info[1],filename)
    ...
    if not os.path.exists(this_saved_file):
        with open(sgyfile,'rb') as file:
            ...
            true_count = 0
            false_count = 0
            for Cur_line in range(line_s + 2, line_e - 25 + 1):
                for Cur_cdp in range(cdp_s + 1, cdp_e - 1 + 1):
                    key = "{}-{}".format(Cur_line,Cur_cdp)
                    if key in loc_time_dict:
                        ...
                        horizon_data.append(np.empty(shape=(slice_step,slice_step,3)))
                        false_count += 1
                    count += 1
                    if count%1000 == 0: print("finished{}".format(count/total_trace))

            print('true: ', true_count)
            print('false: ', false_count)
            with open(this_saved_file, 'wb') as file:
                pickle.dump(horizon_data, file, -1)

    else:
        print(this_saved_file+'finished！')
def save_well_points(total=False):
    ...
    if os.path.exists(well_sample_p_savefile):#系统目录中含有这个文件
        print('%s:finished'%well_sample_p_savefile)
        return
    else:
        print('generating:%s'%well_sample_p_savefile)
    ...
    for line in reservoir_Info_reader:
        ...
        if count%100 == 0:print('finished: %.2f'%(count/3500))
        count += 1
...
def generate_training_samples(filepath,filename):
    print('开始生成训练数据：\n')
...
def generate_test_horizon_samples(plane_info, sgy_info):
    print("plane info:",plane_info)
    print("sgy info: ", sgy_info)

    get_horizon_slice(filepath=plane_info[0], filename=plane_info[1], sgy_info=sgy_info)
if __name__ == '__main__':
    ...
    #filepath3 = '/usr/Shengli/structureseismic/'
    #filename3 = 'petrel_Dip_deviation_attr.sgy'
    filepath2 = '/disk2/Shengli/data/seismicdata/otherseis/'
    filename2 = 'petrel_Time_gain_attr.sgy'
    filepath4 = '/disk2/Shengli/data/seismicdata/ampseimic'
    filename4 = 'petrel_Amplitude_contrast_attr.sgy'
    #filepath5 = '/usr/Shengli/structureseismic/'
    #filename5 = 'petrel_chaos_attr.sgy'
    """ 生成训练样本 """
    generate_training_samples(filepath=filepath2, filename=filename2)
    plane_path = 'data/plane_loc/'
    plane_loc = "ng32sz_grid_28jun_154436.p701"
    #plane_loc = "ng33sz_grid_28jun_154331.p701"
    #plane_loc = "ngs52_grid_28jun_155214.p701"
    sgy_dir = "/disk2/Shengli/data/seismicdata/"
    child_dir = ["otherseis", "structureseismic", "ampseimic", "structureseismic","structureseismic"]
    sgy_dirs = list(map(lambda x:os.path.join(sgy_dir,x), child_dir))
    sgy_names = ["petrel_Time_gain_attr.sgy"]#, "petrel_Dip_deviation_attr.sgy",
                #"petrel_Amplitude_contrast_attr.sgy", "petrel_chaos_attr.sgy",
                #"petrel_3D_curvature_attr.sgy"]

    """ 生成测试样本 """
    for sgy_dir, sgy_name in zip(sgy_dirs, sgy_names):
        generate_test_horizon_samples(plane_info=[plane_path, plane_loc], sgy_info=[sgy_dir, sgy_name])
