...
class ImageReader(object):
    def __init__(self, data_dir, batch_size, partition, method=3, norm='z_score'):
        print('initializing...')
        self._trace_step = 14
        ...
        for filename in os.listdir(self._data_dir):
            # if 'petrel_Time_gain_attr.sgy_pos.td' in filename:
            if 'petrel_Time_gain_attr.sgy_pos_total.td' in filename:
                self._pos_filename = os.path.join(self._data_dir,filename)
                print('_data_pos')
            # elif 'petrel_Time_gain_attr.sgy_neg.td' in filename:
            elif 'petrel_Time_gain_attr.sgy_neg_total.td' in filename:
    ...
    def normalize(self, data, norm='z_score'):
        ...
        :return:
        """
        #norm_paras_file = os.path.join('max_min_mean_std_new.pkl')
    ...
    def turn_into_specific_channel(self, image, norm='z_score'):
        ...
        if self._method == 3:
            ...
            temp = np.zeros(shape=(self._trace_step, self._trace_step))
            temp[:self._trace_step, :self._trace_step] = image[:, :, 0]
            return temp.reshape((self._trace_step, self._trace_step, 1))
    ...
    @property
    def chanel(self): return self._method
...
if __name__ == '__main__':
    filepath = '/disk4/zk/oil_cnn/data/cnn_train'

    image_reader = ImageReader(filepath,batch_size=32,partition=[0.6,0.2,0.2],method=2,norm='z_score')
